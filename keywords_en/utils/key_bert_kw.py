...
class KeyBertKeywords:
    def __init__(self):
        self.model = self.create_model()

    def create_model(self):
        sentence_transformer_model = SentenceTransformer("distilbert-base-nli-mean-tokens", device="cpu")
        model = KeyBERT(model=sentence_transformer_model)
        return model

    def predict(self, text):
        words = self.model.extract_keywords(text, keyphrase_ngram_range=(1,1), stop_words='english')
        keywords = [k for k, i in words]
        return keywords
